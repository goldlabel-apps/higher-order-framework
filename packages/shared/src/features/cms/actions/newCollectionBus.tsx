...
const q = query(collection(db, collectionName))
            ...
            snap.forEach((document) => {
                if (document.id !== '_shape')
                    list = [...list, { id: document.id, data: document.data() }]
                if (document.id === '_shape')
                    dispatch(
                        updateDataBus({
                            collectionName,
            ...
            dispatch(
                updateDataBus({ collectionName, key: 'list', value: list })
            )
            dispatch(
                updateDataBus({ collectionName, key: 'loading', value: false })
            )
            dispatch(
